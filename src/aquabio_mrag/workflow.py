...
from .conversation import ConversationStore, serialized_session
from .cancellation import cancellable, check_cancelled
from .io_utils import read_jsonl
from .models import AquaBioState, RouteDecision
from .retrieval import MultiSourceRetriever, RetrievalRequest
from .retrieval_agent import RetrievalAgent
from .evidence import weighted_rrf
...
from .answers import AnswerNodes
...
class AquaBioMRAGWorkflow(AnswerNodes):
    def __init__(
        self,
        paths: MRAGPaths,
    ...
    def close(self) -> None:
        """Release the checkpoint database after all invocations have finished."""
    ...
    def vision_node(self, state: AquaBioState) -> dict:
        ...
        vision_llm = (
            self.llm
            if vision_provider in {"qwen", "stepfun", "openrouter"}
    ...
    def retrieval_node(self, state: AquaBioState) -> dict:
        ...
        if not self.offline and needs_web_research(original_query):
        ...
        if "pdf_retriever" in state["selected_tools"]:
            if self.offline:
                ...
                try:
                    ...
                    retrieval_warnings.extend(graph_payload.get("warnings", []))
                    graph_call = self.retrieval_agent._mcp_tool_call_record(
                        "raganything_hybrid_search", "raganything", graph_arguments,
                        graph_started, len(graph_rows),
                    )
                    fused_rows = weighted_rrf(
                        {"chroma": self.retrieval_agent._local_evidence(pdf_context_chroma), "graph": graph_rows},
                        {"chroma": 0.4, "graph": 0.6}, top_k=top_k,
                    )
                    pdf_context = []
                        ...
                        pdf_context.append(
                            {
                                ...
                                "metadata": {
                                    **row.metadata,
                                    ...
                                    "retrieval_source": "mcp_lightrag_graph" if row.source == "lightrag_graph" else "mcp_chroma_pdf",
                                },
                            }
                        )
                except Exception as error:
                    graph_call = self.retrieval_agent._mcp_tool_call_record(
                        "raganything_hybrid_search", "raganything", graph_arguments,
                        graph_started, 0, status="failed", error=str(error),
                    )
                ...
                retrieval_meta = {
                    "counts": {
                        ...
                        "graph": graph_call.get("result_count", 0),
    ...
    def memory_save_node(self, state: AquaBioState) -> dict:
        ...
        explicit_taxon = bool(re.search(r"\b[A-Z][a-z]{2,}\s+[a-z][a-z.-]{2,}\b", state.get("original_query", "")))
        if not species_ids and not explicit_taxon:
            counts: dict[str, int] = {}
            for context_name in (
                "text_context",
                "image_context",
        ...
        check_cancelled()
        if state.get("runtime_options", {}).get("defer_memory_save"):
            return {"session_file": "", "memory_summary": summary,
                    "pending_memory_turn": turn, "trace": saved_trace}
        path = self.conversations.append_turn(state.get("session_id", "default"), turn, summary)
        return {"session_file": str(path), "memory_summary": summary, "trace": saved_trace}
    ...
    def invoke(
        ...
        last_progress = None
        for namespace, snapshot in self.graph.stream(
            initial,
            ...
            if progress_callback and trace and trace[-1] != last_progress:
                last_progress = trace[-1]
