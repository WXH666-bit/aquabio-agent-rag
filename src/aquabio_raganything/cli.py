...
def main() -> int:
    ...
    parser.add_argument("--root", type=Path, default=Path.cwd(), help="Project data directory")
    ...
    native_index.add_argument("--unit", help="Index one exact book-native unit")
    ...
    if args.command == "inventory":
        ...
        result = asyncio.run(index_segments(
            paths, settings, args.scope, resume=args.resume,
            limit=args.limit, segment_id=args.segment,
        ))
        _print(result)
        return 1 if result["failed"] else 0
    elif args.command == "index-book-native":
        result = asyncio.run(index_book_native_units(
            paths, settings, args.book, resume=args.resume,
            limit_units=args.limit_units, unit_id=args.unit,
        ))
        _print(result)
        return 1 if result["failed"] else 0
